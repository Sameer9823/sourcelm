...
import { enqueueArtifactGeneration } from "../lib/artifact-events";
import {
...
import { syncReviewCardsFromArtifact } from "./review.service";
import type { CreateArtifactInput } from "../validators/artifact.validator";
...
export async function createArtifactForWorkspace(
    ...
    await spendCredits({
        ...
        description: `Generated ${input.type.toLowerCase()} artifact`,
    });
...
export async function processArtifactById(artifactId: string) {
    ...
    try {
        ...
        const ready = await updateArtifactRecord(artifactId, {
            status: "READY",
        ...
        // Flashcards immediately enter the spaced-repetition review queue —
        // no extra step for the learner.
        await syncReviewCardsFromArtifact(ready);
