...
import { enqueueConversationSummarize } from "../lib/conversation-events";
import {
...
import { addMemoriesFromMessages, searchUserMemories } from "../lib/mem0";
 ...
 * @returns A standard `Response` streaming the UI message stream; sets `X-Conversation-Id` header
 ...
 * @throws {InsufficientCreditsError} When the user is out of credits
 *
 */
export async function streamWorkspaceChat(
    ...
    // Resolve conversation first (validates existence if conversationId provided)
    ...
    // Charge credits after validation, so invalid requests (e.g., non-existent conversation) are free
    // but every real LLM call is guaranteed to have been paid for.
    await spendCredits({
        userId,
        action: "CHAT_MESSAGE",
